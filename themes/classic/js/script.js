...
$(function(){
    ...
    $('body').on('click', '.btn-step', function(){
        ...
        if(step == 1 && nav == 1){
            ...
            var btid = $.trim($('#Customers_business_type_id').val()), bname = $.trim($('#Customers_business_name').val()),
            contactPerson = $.trim($('#Customers_contact_person').val()), address = $.trim($('#Customers_address').val()),
            email = $.trim($('#Customers_email').val());

            if(btid == ''){
                $('.next-error').html('Error: Please select business type!').fadeIn(200);
                step = 3;
                error = 1;
            }else if(bname == ''){
                $('.next-error').html('Error: Please enter your business name!').fadeIn(200);
                step = 3;
                error = 1;
            }else if(contactPerson == ''){
                $('.next-error').html('Error: Please enter your contact person!').fadeIn(200);
                step = 3;
                error = 1;
            }else if(address == ''){
                $('.next-error').html('Error: Please enter your address!').fadeIn(200);
                step = 3;
                error = 1;
            }else if(email == '' || !isValidEmailAddress(email)){
                $('.next-error').html('Error: Please enter your email!').fadeIn(200);
                step = 3;
                error = 1;
            }else{
                $('.next-error').fadeOut(200);
