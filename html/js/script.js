$(function(){
	...
	$('.new-checkbox').click(function(){
		...
		if($this_val == 'small'){
			...
			$('.price-option-preview img').attr('src', 'images/small.png');
		}else if($this_val == 'large'){
			$('.zoom strong').html('Large');
			$('.price-option-preview img').attr('src', 'images/large.png');
		}else{
			getvals();
		}
	});
	...
	$('body').on('click', '.btn-step', function(){
		...
		if(error == 0){
			...
			$('.the-show').animate({'height': height}, 200);
			$('.form-step-'+step).stop().animate({'opacity': 0}, 400);
			$('.form-step-'+(step+1)).stop().animate({'opacity': 1}, 400);
			$('.step-next').removeClass('ok-step-next');
			$('.step-back').removeClass('ok-step-back');
			$('.sign-up-progress').stop().animate({'left': -step*1133}, 400, function(){
